...
function createRoom() {
	...
	var newRoomObject = {
		...
		currentGame: games.winterGame(), //DEFAULT: Snake | TODO: Implement interface to choose a game once more games are added
		isPaused: true,
		...
		runPauseChecks: function() { //Note: If any players are moved in any of these checks, the checks may have to be run again to ensure that the new circumstances do not necessitate a pause.
			if (this.isPaused) {
				...
				if (this.pauseReasons.length == 0) {
					this.lastTickTime = getTime(); //Prevent games from responding to what may be perceived as a lag in ticks when in reality the game was just paused.
					this.isPaused = false;
				}
			}
			else {
